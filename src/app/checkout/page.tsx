...
import { ChevronLeft, CheckCircle, CreditCard, Lock, ShieldCheck } from "lucide-react";
...
export default function CheckoutPage() {
  ...
  // Restore normal scroll if it was stopped
  useEffect(() => {
  ...
  const handleCloseSuccess = () => {
    setIsSuccess(false);
    clearCart();
    // Redirect is handled by the Link wrapper, but we clear first
  };
        ...
        <AnimatePresence mode="wait">
          {isSuccess ? (
            ...
            <motion.div 
              ...
              exit={{ opacity: 0, y: -20 }}
              className="flex flex-col items-center justify-center text-center p-12 md:p-24 bg-white border border-black/5 rounded-[40px] shadow-sm gap-6"
            >
              <motion.div
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
            ...
            >
              {/* Left Side: Forms */}
              <form onSubmit={handleSubmit} className="flex-1 bg-white border border-black/5 p-8 md:p-12 rounded-[40px] shadow-sm">
                <div className="flex flex-col gap-8">
                  <div>
                    <span className="text-xs font-black tracking-widest text-black/40 block mb-1">// SECURE CHECKOUT</span>
                    <h3 className="text-3xl font-black tracking-tight uppercase">TRANSACTION DETAILS</h3>
                  </div>
              ...
              <div className="w-full lg:w-96 bg-white border border-black/5 p-8 md:p-12 rounded-[40px] shadow-sm flex flex-col justify-between h-fit self-start">
                <div className="flex flex-col gap-6">
                  <h4 className="text-xs font-black tracking-widest text-black/50 border-b border-black/5 pb-3 uppercase">ORDER SUMMARY</h4>
                  
                  {/* Items List */}
                  <div className="flex flex-col gap-5 max-h-[40vh] overflow-y-auto pr-2">
                    {cart.length === 0 ? (
                      <p className="text-xs font-semibold text-black/40 py-4">No items in your bag.</p>
                    ) : (
                      cart.map((item) => (
                        <div key={`${item.id}-${item.size}`} className="flex gap-4 items-center">
                          <div className="relative w-14 h-14 bg-black/5 border border-black/5 rounded-2xl flex items-center justify-center p-1 shrink-0">
                            <Image 
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <span className="text-[9px] font-black tracking-widest text-black/40 uppercase block mt-2">
                ...
                {/* Receipt Calculation */}
                <div className="border-t border-black/5 pt-6 mt-8 flex flex-col gap-4">
